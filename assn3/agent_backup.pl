...
s(goal(X0, Y0), goal(X1, Y1), 1) :-
    % land(X1, Y1),
    write('X: '), write(X1), writeln(''),
    write('Y: '), write(Y1), writeln(''),
...
% Case 2: Head_plan is not applicable, find a new plan, append to Plan_1 at head.
